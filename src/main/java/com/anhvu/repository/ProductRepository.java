...
public interface ProductRepository extends JpaRepository<Products, Long> {
	...
	@CachePut(value = "productsCache", key = "#product.idProducts")
	Products save(Products product);

	@CacheEvict(value = "productsCache", key = "#id")
	void deleteById(Long id);

	@Query(value = "EXPLAIN SELECT p FROM Products p WHERE p.idProducts = (SELECT MAX(p2.idProducts) FROM Products p2)", nativeQuery = true)
